...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, causal_mask=None, padding_mask=None):
        ...
        # Restore original shape and project
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.act = nn.GELU()  # Changed from GELU to SiLU
...
class Block(nn.Module):
    ...
    def forward(self, x, encoder_out=None, causal_mask=None, padding_mask=None):
        x = x + self.attn(self.rms_1(x), causal_mask, padding_mask)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_out=None, padding_mask=None):
        x = x + self.self_attn(self.rms_1(x))
...
class EncoderDecoderGPT(nn.Module):
    def __init__(self, config):
        ...
        self.rms_final = RMSNorm(config.n_embd)  # Changed from LayerNorm to RMSNorm
...
wandb_log = True
wandb_project = 'owt'
wandb_run_name = 'base-transformer-gelu'
...
# analyzer.analyze_model(use_nGPT=False, iter_num=iter_num, wandb_log=wandb_log)
